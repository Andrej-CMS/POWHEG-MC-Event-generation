...
c CAVEAT!!!  process dependent subroutine
      ...
      if(ini) then
         ...
         if(muref.lt.0) then
            ...
            write(*,*) '****************************************'
            write(*,*) '****************************************'
            write(*,*) '**   mur=ET  used for Bbar function   **'
            write(*,*) '**   muf=ET  used for Bbar function   **'
            write(*,*) '****************************************'
            write(*,*) '****************************************'
         endif
         ini=.false.
      endif
c     running scale is ET/2 (scalar sum of energies of top quark antiquark system + jet)
      if(.not.fixedscale) then

        muref=sqrt(kn_pborn(1,3)**2+kn_pborn(2,3)**2+kn_masses(3)**2)
        muref=muref+sqrt(kn_pborn(1,4)**2+kn_pborn(2,4)**2+kn_masses(4)**2)
        muref=muref+sqrt(kn_pborn(1,5)**2+kn_pborn(2,5)**2+kn_masses(5)**2)
        muref=muref/2.d0
c        WRITE (*,*)'top mass =',kn_masses(3)
c        WRITE (*,*)'anti-top mass =',kn_masses(4)
c        WRITE (*,*)'jet mass =',kn_masses(5)
c        WRITE (*,*)'muref =',muref
      endif
      muf=muref
      mur=muref
