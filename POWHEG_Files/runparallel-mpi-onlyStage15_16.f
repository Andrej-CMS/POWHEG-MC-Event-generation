...
C       call runcommand(trim(main),rank,1,1)
C       call mpi_barrier(mpi_comm_world,ierr)

C c     stage 1, iteration 2

C       call runcommand(trim(main),rank,1,2)
C       call mpi_barrier(mpi_comm_world,ierr)

C c     stage 1, iteration 3

C       call runcommand(trim(main),rank,1,3)
C       call mpi_barrier(mpi_comm_world,ierr)

C c     stage 1, iteration 4

C       call runcommand(trim(main),rank,1,4)
C       call mpi_barrier(mpi_comm_world,ierr)

c     stage 1, iteration 5

      call runcommand(trim(main),rank,1,5)
      call mpi_barrier(mpi_comm_world,ierr)

c     stage 1, iteration 6

      call runcommand(trim(main),rank,1,6)
      call mpi_barrier(mpi_comm_world,ierr)

c     stage 2
C       call DATE_AND_TIME(date, time, zone, values)
C       write(iun,12) 'Start stage 2, ',values(3),'/',values(2),'/',
C      1 values(1),' ',values(5),':',values(6),':',values(7)

C       call runcommand(trim(main),rank,2,0)

C       call mpi_barrier(mpi_comm_world,ierr)

c     stage 3
C_EFT      call DATE_AND_TIME(date, time, zone, values)
C_EFT      write(iun,12) 'Start stage 3, ',values(3),'/',values(2),'/',
C_EFT     1     values(1),' ',values(5),':',values(6),':',values(7)

C_EFT      call runcommand(trim(main),rank,3,0)

C_EFT      call mpi_barrier(mpi_comm_world,ierr)
      
c     stage 4
C_EFT      call DATE_AND_TIME(date, time, zone, values)
C_EFT      write(iun,12) 'Start stage 4, ',values(3),'/',values(2),'/',
C_EFT     1     values(1),' ',values(5),':',values(6),':',values(7)

C_EFT      call runcommand(trim(main),rank,4,0)
